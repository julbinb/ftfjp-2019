...
which requires ``coming up'' with a middle type $\ty_2$.
...
The syntax-directed reductive definition of subtyping
is presented in~\figref{fig:bjsem-red-sub}.\footnote{The definition
%the rules can be easily turned 
%into a subtyping algorithm.
	 is not deterministic.	For example, there are two ways to derive 
	$\bjsubr{\typair{\tystr}{(\tyunion{\tyint}{\tyflt})}}
	{\typair{\tystr}{(\tyunion{\tyint}{\tyflt})}}$: 
	either by immediately applying \RR{Pair}, 
	or by first normalizing the left-hand side with \RR{NF}.}
Some of the inductive rules are similar to their declarative counterparts,
e.g. subtyping of pairs (\RR{Pair}) and a union on the left (\RR{UnionL}).
The differing rules are \colorbox{light-gray}{highlighted}.
The explicit reflexivity rule \RR{BaseRefl} now works only with 
...
and a union on the right (\RR{UnionR1}, \RR{UnionR2}).

The last rule of the definition, \RR{NF}, is the most important.
It rewrites type \ty into \emph{disjunctive normal form} $\NF(\ty)$
before using other subtyping rules.
This covers all useful applications of transitivity and distributivity 
that are possible in the declarative definition.
The normalization function is presented in~\figref{fig:bjsem-calc-nf}
(the auxiliary function $\unprs$ 
can be found in~\figref{fig:bjsem-calc-nf-full}, \appref{app:nf}).
It produces a type of the form
$\vty_1 \cup \vty_2 \cup \ldots \cup \vty_n$ (note that union is associative)
be replacing an abstract nominal type 
...
\]
As we show in~\secref{}, a type and its normal form are
equivalent in the declarative definition, which is essential
