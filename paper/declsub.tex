...
In the finite case, directly manipulating interpretations
and checking the subset relation on them would be extremely inefficient.
%,\eqref{eq:semsub-def} 
%cannot be directly computed in the general case.
Therefore, we provide an alternative, \emph{syntactic} definition of subtyping 
that is equivalent to~\eqref{eq:truesemsub-def}
and also straightforward to implement.
