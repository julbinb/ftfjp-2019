...
(** * FullAtomicJl: Simple Propositions *)
(* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% *)
...
Lemma atom__valid : forall (t : ty),
    atom_type t ->
    valid t.
Proof.
  intros t Hat; induction Hat; constructor; assumption.
Qed.

Lemma nf__valid : forall (t : ty),
    InNF(t) ->
    valid t.
Proof.
  intros t Hnf; induction Hnf.
  apply atom__valid; assumption.
  constructor; assumption.
Qed.

Lemma value_type_valid__atom : forall (v : ty),
    value_type v -> valid v ->
    atom_type v.
Proof.
  intros v Hval HTYv. induction Hval.
  - (* CName *)
    constructor.
  - (* Pair *)
...
Lemma unite_pairs_of_valuetys : forall (ta1 ta2 : ty),
    value_type ta1 ->
    value_type ta2 ->
    unite_pairs ta1 ta2 = TPair ta1 ta2.
Proof.
...
(* DEP: unite_pairs_of_valuetys *)
Lemma mk_nf_valuety__equal : forall (v : ty),
    value_type v ->
    MkNF(v) = v.
Proof.
  intros v Hval; induction Hval;
  ...
  rewrite unite_pairs_of_valuetys; try assumption.
  reflexivity.
Qed.

(*
Lemma unite_pairs__preserves_valid : forall (t1 t2 : ty),
    valid t1 ->
    valid t2 ->
    valid (unite_pairs t1 t2).
Proof.
  intros ta tb HTYa. generalize dependent tb.
  induction HTYa;
    intros tb HTYb; induction HTYb;
      try solve [simpl; repeat constructor].
  simpl.
Abort.
*)
...
Proof.
  ...
  - (* AName *)
    destruct an; simpl; repeat apply NF_Union; repeat constructor.
  - (* Pair *)
...
Theorem mk_nf__idempotent : forall (t : ty),
    valid t ->
    MkNF( MkNF(t) ) = MkNF(t).
Proof.
  intros t HTYt. apply mk_nf_nf__equal.
  apply mk_nf__in_nf; assumption.
Qed.

(* DEP: mk_nf__in_nf, nf__valid *)
Lemma mk_nf__preserves_valid : forall (t : ty),
    valid t ->
    valid (MkNF(t)).
Proof.
  intros t HTYt.
  pose proof (mk_nf__in_nf t HTYt).
  apply nf__valid; assumption.
Qed.

Lemma mk_nf__preserves_value_type : forall (t : ty),
    value_type t ->
    value_type (MkNF(t)).
Proof.
  intros t Hval; induction Hval;
    try solve [simpl; constructor].
  simpl. rewrite unite_pairs_of_valuetys; try assumption.
  constructor; assumption.
Qed.
...
Lemma sub_r__rflxv : forall (t : ty),
    ...
    |- t << t.
Proof.
  intros t HTY; induction HTY; try solve [constructor; assumption].
  (* Union *)
  constructor;
